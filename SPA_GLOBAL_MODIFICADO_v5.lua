		...
		for _, v in ipairs(root:GetDescendants()) do
			if (v:IsA("NumberValue") or v:IsA("IntValue")) and v.Value > 0 then
				...
				if nm=="maxspeed" or nm=="topspeed" or nm=="top_speed" or nm=="speedlimit" then
					return mfloor(v.Value * 10 + 0.5) / 10
				end
			end
		end
	end
...
local function makeVueltasRow(parent, order, pos, p)
	...
	vueltasRowCache[uid] = {topRow = topRow, btnRow = btnRow}
end
...
function _setupCollisionDetection()
	...
	-- ── Heartbeat de detección (separado del principal) ──────────
	local _tCrash = 0
	RunService.Heartbeat:Connect(function(dt)
		_tCrash += dt
		if _tCrash < 0.08 then return end   -- [OPT] muestreo cada 80 ms para bajar carga
		_tCrash = 0
		...
		if lat and not lat._rpDone and _rpCapture then
			...
			for _, pl in ipairs(Players:GetPlayers()) do
				...
				if dn == lat.nameA then
					...
					-- [PRECISION FIX] usar posición del vehículo si está en uno
					local hum2 = char and char:FindFirstChildOfClass("Humanoid")
					local seat2 = hum2 and hum2.SeatPart
					if seat2 and seat2:IsA("VehicleSeat") and seat2.Occupant == hum2 then
						posA = seat2.Position
					else
...
function _setupAnalysisDetection()
	...
	-- Aviso informativo en la parte superior de ANÁLISIS
	...
	_analHintLbl.Text = "ℹ️  Para ver los choques debes estar encima de un vehículo"
	...
	local function rebuildAnalUI()
		-- Solo replays: limpiar todo excepto UIListLayout y tarjetas de replay
		...
		-- Mensaje cuando no hay eventos grabados aún
		...
		if not _hasCards then
			...
			_el.Text="📹  Sin contactos grabados aún.\n\nSe graban automáticamente cuando dos pilotos en vehículo se acercan a menos de 35 studs, o cuando el sistema de choques detecta un impacto."
...
SPA_Telemetry = {
	...
	alerts       = {},
	-- [DRIFT FIX] latestDrift[uid] = fricción más reciente detectada por señal en tiempo real.
	-- Se actualiza SOLO cuando una llanta cambia de CustomPhysicalProperties (evento reactivo),
	-- evitando el problema de que el escaneo estático lea 0.6 o 1.5 según el frame del físico.
	latestDrift  = {},
	-- [DRIFT FIX] conexiones activas por uid (para desconectar al salir del vehículo)
	driftConns   = {},
}
...
function _telGetDrift(seat, uid)
	-- [DRIFT FIX] Consulta primero el valor reactivo (evento en tiempo real).
	-- Este valor solo cambia cuando la llanta REALMENTE cambia su fricción,
	-- eliminando el salto 0.6 <-> 1.5 que producía el escaneo estático por frame.
	if uid and SPA_Telemetry.latestDrift[uid] ~= nil then
		return _telFormatDrift(SPA_Telemetry.latestDrift[uid])
	end
	-- Fallback: escaneo estático (antes del primer evento o si uid no disponible)
	local root = _telGetRootModel(seat)
...
-- [DRIFT FIX] Conecta señales reactivas de fricción para un vehículo.
-- Desconecta las conexiones previas del uid antes de crear nuevas.
local function _telWatchDrift(uid, seat)
	...
	SPA_Telemetry.latestDrift[uid] = nil  -- reiniciar al entrar a un vehículo nuevo
			...
			local sigOk, conn = pcall(function()
				return part:GetPropertyChangedSignal("CustomPhysicalProperties"):Connect(function()
					...
					if mabs(newFr - frCache) > 0.001 then
						frCache = newFr
						-- [DRIFT FIX v2] seguir EXACTAMENTE el último valor que cambió,
						-- igual que tu script de referencia. No conservar el más desviado,
						-- porque eso dejaba pegado 1.5 aunque luego bajara a 0.6.
...
-- [DRIFT FIX] Desconecta y limpia todo al salir del vehículo
...
SPA_Tires = {
	...
	MAX_LOG      = 80,
	rebuildFn    = nil,
	-- [SPAM FIX] tiempo que un compuesto debe estar estable antes de notificar
	STABLE_TIME  = 2.0,   -- segundos de estabilidad antes de disparar notificación
...
-- ════════════════════════════════════════════════════════════════
SPA_Replay = {
	RADIUS     = 20,    -- [OPT] radio más selectivo: menos comparaciones, menos lag
	BUF_SEC    = 5.0,   -- segundos de historia en el ring buffer
	POST_SEC   = 3.0,   -- [POST] segundos que se siguen grabando después del impacto
	BUF_HZ     = 12,    -- [OPT] menos muestras/segundo para reducir CPU y memoria
...
function _rpBufPush(uid, pos, vel, rot)
	...
	-- rot = CFrame de rotación del vehículo (sin traslación), para replay fiel
	b[#b + 1] = { pos = pos, vel = vel, rot = rot, t = tick() }
	if #b > maxN then table.remove(b, 1) end
end

-- Devuelve copia del buffer actual (orden cronológico)
...
function _rpCapture(uidA, nameA, uidB, nameB, dv, posImp)
	...
	-- [POST FIX] Guardamos el buffer PRE impacto ahora y, tras POST_SEC,
	-- añadimos solo las muestras posteriores al impacto para ver qué pasó después.
	local impactAt = now
...
function _rpPlay3D(ev, cloneA, partsA, cloneB, partsB)
	...
	local REPLAY_Y_OFFSET = Vector3.new(0, 3.5, 0)  -- [FIX] elevar replay para evitar que quede bajo el mapa
	...
	-- [ROT FIX] Interpolación esférica de rotación (CFrame slerp)
	local function rotAt(tl, t)
		...
		for i=1,#tl-1 do
			if tl[i].t<=t and tl[i+1].t>t then
				...
				-- CFrame:Lerp hace slerp en rotación y lerp en posición de Roblox
	...
	local t0rs = tick()
	local _3d_acc = 0
	local _3d_interval = 1/30  -- exactamente 30 FPS, no más
	local _rc; _rc = RunService.RenderStepped:Connect(function(dt)
		...
		local function moveCl(cl, anc, off, tl)
			...
			local cf
			-- [ROT FIX] Usar rotación grabada si está disponible (replica movimiento exacto)
			local rot = rotAt(tl, t)
			if rot then
				-- Aplicar traslación actual a la rotación grabada (rot ya viene sin posición)
				cf = CFrame.new(pos) * rot
			else
				-- Fallback: orientar según dirección de movimiento (comportamiento anterior)
		...
		if t>dur+0.8 then
			...
			-- [FIX] restaurar SIEMPRE el control de cámara al humanoide local
			do
...
function _setupReplayDetection()
	...
	RunService.Heartbeat:Connect(function(dt)
		...
		for _, p in ipairs(Players:GetPlayers()) do
			...
			local hrp = char:FindFirstChild("HumanoidRootPart")
			if not hrp then continue end
			-- [PRECISION FIX] Si el piloto está en vehículo, usar la posición
			-- del seat (centro del coche) en vez del HRP del personaje.
			-- Esto hace que los replays sean idénticos a la posición real del vehículo.
			local hum = char:FindFirstChildOfClass("Humanoid")
			local seat = hum and hum.SeatPart
			local usePos, useVel
			if seat and seat:IsA("VehicleSeat") and seat.Occupant == hum then
				usePos = seat.Position
				useVel = seat.AssemblyLinearVelocity
				-- [ROT FIX] capturar rotación real del vehículo para replay fiel
				local rotOnly = nil
				do
				...
				_rpBufPush(p.UserId, usePos, useVel, rotOnly)
			else
				usePos = hrp.Position
				useVel = hrp.AssemblyLinearVelocity
				_rpBufPush(p.UserId, usePos, useVel, nil)
			end
		end
	end)

	-- ── 2) Proximidad + pequeño‑Δv (0.1 s) ───────────────────────
	local _tPrx, _prevV = 0, {}
	RunService.Heartbeat:Connect(function(dt)
		_tPrx += dt
		if _tPrx < 0.15 then return end  -- [OPT] revisar proximidad menos veces
		_tPrx = 0
		local allPl = Players:GetPlayers()
		...
		for _, p in ipairs(allPl) do
			...
			-- [PRECISION FIX] usar seat.Position (centro del coche) para posición exacta
			curP[uid] = seat.Position
		end
	...
	-- [OPT] Render 2D eliminado para ahorrar UI/CPU. Solo replay 3D.
	local function renderCanvas(canvas, ev, animated)
		...
		msg.Text = "Replay 2D desactivado\nUsa el botón REPRODUCIR 3D"
	...
	local function buildCard(ev, order)
		...
		gBtn.MouseButton1Click:Connect(function()
			...
			local function mkGhost(pl, col, snap)
				-- [FIX] Si el jugador sigue en servidor, intentar clonar su vehículo actual.
				-- Si ya se fue o no se puede clonar, usar un ghost liviano basado SOLO en snapshots.
				if pl then
				...
				-- [FIX] Ghost independiente del jugador: funciona aunque se salga del servidor.
...
function _setupUI2()  -- [SPAV4] global: libera registros del ámbito principal
	...
	local lastSeenInLap    = {}
	local lastSeenInPitIn  = {}
	local lastSeenInPitOut = {}
	...
	RunService.Heartbeat:Connect(function(dt)
		...
		-- [OPTIMIZACIÓN] Actualizamos el filtro para solo escanear personajes en este frame (Cero Lag)
		...
		if _tHead >= 0.06 then
			...
			for _, p in ipairs(Players:GetPlayers()) do
				...
				if seat and seat:IsA("VehicleSeat") and seat.Occupant == humanoid then
					-- [DRIFT FIX] Detectar cambio de vehículo y reconectar monitor de fricción
					local uid = p.UserId
					if not SPA_Telemetry.driftConns[uid] or
					   (SPA_Telemetry.driftConns[uid]._seat ~= seat) then
						_telWatchDrift(uid, seat)
						SPA_Telemetry.driftConns[uid]._seat = seat
					end
					...
					if tag then
						...
						if speedLbl then
							...
							-- Notificación: MaxSpeed vs límite efectivo
							if maxSpd > effLim then
								if not alertedPlayers[uid] then
									alertedPlayers[uid] = true
						...
						if telLbl and SHOW_HEAD_SPEED then
							...
							local driftV = _telGetDrift(seat, p.UserId)  -- [DRIFT FIX] uid reactivo
					...
					-- [DRIFT FIX] Al salir del vehículo, limpiar monitor de fricción
					_telStopDrift(p.UserId)
					removeSpeedTag(char)
		...
		if _tLap >= 0.1 then
			...
			-- ── Detección Vueltas/Boxes ──
			if DETECT_LAPS and lapWall then
				local inside = playersInPart(lapWall)
				for _, pl in ipairs(Players:GetPlayers()) do
					ensurePlayerData(pl)
					local uid = pl.UserId
					local ld  = lapData[uid]
					local fld = fastLapData[uid]
					local wasIn = lastSeenInLap[uid] or false
					local isIn  = inside[uid] or false
					if isIn and not wasIn then
						if not FIA_EXCLUDED[uid] then
							local now = tick()
							if now-(ld.lastLapTouch or 0) >= DEBOUNCE_TIME then
								...
								if fld.currentLapStarted and fld.lastStartTime then
									...
									if not fld.bestTime or lapTime < fld.bestTime then fld.bestTime = lapTime end
								end
					...
					lastSeenInLap[uid] = isIn
				end
			end

			if DETECT_PITS and pitInWall and pitOutWall then
				local insideIn  = playersInPart(pitInWall)
				local insideOut = playersInPart(pitOutWall)
				for _, pl in ipairs(Players:GetPlayers()) do
					ensurePlayerData(pl)
					local uid = pl.UserId
					local pd  = pitData[uid]
					if not FIA_EXCLUDED[uid] then
						if (insideIn[uid] or false) and not (lastSeenInPitIn[uid] or false) then
							if pd.status == "En Pista" then pd.status = "En Boxes"; pd.lastPitTouch= tick() end
						end
						if (insideOut[uid] or false) and not (lastSeenInPitOut[uid] or false) then
					...
					lastSeenInPitIn[uid]  = insideIn[uid]  or false
					lastSeenInPitOut[uid] = insideOut[uid] or false
				end
			end

			-- ── Detección Corner Cut Integrada ──
			if DETECTCC then
				for id, entry in pairs(ccWaypoints) do
					if entry.wall and entry.wall.Parent then
						local inside = playersInPart(entry.wall)
						if not lastSeenInCC[id] then lastSeenInCC[id] = {} end
						for _, pl in ipairs(Players:GetPlayers()) do
							local uid = pl.UserId
							local wasIn = lastSeenInCC[id][uid] or false
							local isIn  = inside[uid] or false
	...
	Players.PlayerRemoving:Connect(function(pl)
		...
		lastSeenInLap[uid]=nil; lastSeenInPitIn[uid]=nil; lastSeenInPitOut[uid]=nil
...
function _setupTireSystem()
	...
	-- Cada ~0.5 s lee el VehicleSeat activo de cada piloto.
	-- Si detecta un compuesto reconocido y distinto al registrado → loguea.
	local _tTire = 0
	RunService.Heartbeat:Connect(function(dt)
		_tTire += dt
		...
		for _, p in ipairs(Players:GetPlayers()) do
			...
			if not prev or prev.name ~= cpd.name then
				-- [SPAM FIX] No notificar de inmediato: esperar que el piloto
				-- lleve STABLE_TIME segundos en el mismo compuesto para evitar
				-- spam al pasar por compuestos intermedios.
				local st = SPA_Tires._stableTimer[uid]
				if not st or st.cpd ~= cpd.name then
					-- Compuesto nuevo o cambió: reiniciar temporizador
					SPA_Tires._stableTimer[uid] = { cpd = cpd.name, since = tick() }
				else
					-- Mismo compuesto: comprobar si ya pasó STABLE_TIME
					if tick() - st.since >= SPA_Tires.STABLE_TIME then
						-- Estable → registrar y notificar UNA sola vez
				...
				-- Compuesto igual al registrado → limpiar temporizador pendiente
				SPA_Tires._stableTimer[uid] = nil
			end
		end
	end)
	...
	Players.PlayerRemoving:Connect(function(pl)
		...
		SPA_Tires._stableTimer[pl.UserId] = nil  -- [SPAM FIX] limpiar temporizador
		_telStopDrift(pl.UserId)                 -- [DRIFT FIX] limpiar monitor de fricción
	end)
